...
**简历和密钥不出本机。** 负责写话术的本地服务只监听 `127.0.0.1`，密钥、简历、投递记录都已经 gitignore，不会进仓库。
...
| 维度 | 分值 |
|---|---|
| 方向与技能词命中 | 最多加 60 分，同样的词出现在标题里权重翻倍 |
| 标题强化词命中 | 最多加 20 分，标题里直接写着目标方向或「研究员」「数据科学」这类词 |
...
方向词表、标题强化词表、排除词表都是 `matcher.py` 顶部的普通常量，想换行业直接改这几个列表即可，不需要动逻辑。
...
注意 BOSS 的路径是 `jobs` 复数，写成单数 `job` 会零结果或者跳回首页。城市码示例：全国 `100010000`，温州 `101210700`。

## 用法

1. 给 Edge 装 Tampermonkey，再导入 `auto_job_find/boss_ai_helper.user.js`。
2. 双击 `auto_job_find/start_ai_server.bat`，窗口保持开着，它负责本地写话术的服务。
3. 打开岗位搜索结果页，并按下节把 `.env` 配好。
4. 先点面板上的 **◐ 先演练一遍**，日志区会逐个岗位告诉你得了多少分、判成什么、为什么。
5. 确认判断符合预期后，点 **▶ 一键自动：筛完就投**。

更细的说明见 `auto_job_find/油猴助手使用说明.md`，里面有筛选规则、安全闸门和离线自测方法。
...
| `INCLUDE_KEYWORDS` / `EXCLUDE_KEYWORDS` | 临时加的包含和排除词，留空则不生效 |
